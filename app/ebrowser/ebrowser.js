...
//***************************************************************************
{
	...
	eBrowser.createUi = function () {
		...
		window.tabApi.checkUpdate().then((hasUpdate)=>{
			if(hasUpdate) {
...
window.tabApi.onDownloadUpdate((index)=>{
	browser.navi.downloadUpdate();
});

window.tabApi.onUpdateReady((index)=>{
	browser.navi.updateReady();
});
